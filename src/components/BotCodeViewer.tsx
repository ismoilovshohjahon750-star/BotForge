...
export const BotCodeViewer: React.FC<BotCodeViewerProps> = ({ files, secrets = [] }) => {
  ...
  useEffect(() => {
    ...
    setBotName(meta.language === 'python' ? 'Python Aqlli Agent Bot' : 'BotForge Node.js Bot');
  }, [localFiles]);
  ...
  const handleDownloadZip = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `${botName.toLowerCase().replace(/\s+/g, '-') || 'botforge'}-code.zip`;
    ...
    <div id="bot-code-viewer" className="mt-4 w-full bg-[#0d0d14] rounded-2xl border border-white/10 overflow-hidden shadow-2xl transition-all duration-300 relative group/viewer">
      ...
      {showDeployModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-md" onClick={() => !isDeploying && setShowDeployModal(false)} />
          
          <div className="relative w-full max-w-lg bg-[#0d0d14] border border-white/10 rounded-2xl overflow-hidden shadow-2xl p-6 text-left">
            <div className="flex items-center gap-3 border-b border-white/5 pb-4 mb-5">
              ...
              <div>
                <h4 className="text-sm font-semibold text-white">BotForge Hostingga Deploy Qilish</h4>
            ...
            {!isDeploying ? (
              ...
              <div className="space-y-5 text-center py-6">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-[10px] text-slate-500 font-mono">Server ID: botforge-cloud-run-container</p>
                </div>
